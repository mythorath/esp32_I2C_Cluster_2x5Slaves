...
// poolDifficulty is the REAL share difficulty the pool assigns (mining.set_difficulty).
// A hash must beat this to be worth submitting / accepted by the pool.
double poolDifficulty = 0.00005;

// displayDifficulty is a deliberately-EASY local target handed to the slaves so the
// LCD / web "shares found" counter and the celebration animation stay lively. Slaves
// report every hash that beats this; the master then forwards to the pool only the
// rare ones that ALSO beat poolDifficulty, so the pool never sees junk shares.
// Pacing: roughly  totalHashrate / (displayDifficulty * 2^32)  finds per second.
// At ~185 kH/s, 0.0005 -> ~1 hit every ~11 s cluster-wide (negligible hashrate cost).
double displayDifficulty = 0.0005;

uint32_t totalHashrate = 0;
uint32_t sharesFound = 0;
uint32_t sharesAccepted = 0;
uint32_t sharesRejected = 0;
uint8_t shareTarget[32];     // easy DISPLAY target actually sent to the slaves
uint8_t poolTargetBE[32];    // real POOL target (big-endian) used to gate submissions
...
// Does a slave-reported double-SHA256 hash beat the REAL pool target?
// This mirrors the slave's comparison exactly. The slave stores its target
// reversed (little-endian) and walks from the most-significant byte down;
// poolTargetBE is big-endian (index 0 = MSB), so we reverse it the same way.
bool hashMeetsPoolTarget(const uint8_t* hash) {
    for(int i = 31; i >= 0; i--) {
        uint8_t t = poolTargetBE[31 - i];   // reversed -> matches the hash byte order
        if(hash[i] < t) return true;
        if(hash[i] > t) return false;
    }
    return false;  // exactly equal is not strictly below target
}
...
void handleStratumSetDifficulty(JsonArray& params) {
    ...
    // Pool difficulty only changes what we SUBMIT (poolTargetBE). The slaves keep
    // mining the constant easy displayDifficulty target, so there is no need to
    // re-push the current job here.
    difficultyToTarget(poolDifficulty, poolTargetBE);
...
// ============================================================
// The dashboard markup lives in index_html.h (see that file for why it
// cannot be an inline raw string in this .ino). It defines INDEX_HTML.
            ...
            if(status == RESP_FOUND && bytesRead >= 45) {
                ...
                if(jobId == currentJob.jobIdNum && 
                   !(nonce == slaves[i].lastSubmittedNonce && jobId == slaves[i].lastSubmittedJobId)) {
                    ...
                    // Every reported hit already beats the easy DISPLAY target, so
                    // count it and light up the LCD/web dashboard.
                    sharesFound++;
                    memcpy(lastFoundHash, hash, 32);   // stash for the LCD celebration
                    haveFoundHash = true;

                    // Only the rare hits that ALSO beat the real pool target get
                    // submitted, so the pool never receives low-difficulty junk.
                    if(hashMeetsPoolTarget(hash)) {
                        stratumSubmit(currentJob.jobId, nonce, currentJob.extraNonce2);
                        Serial.printf("*** POOL SHARE submitted by 0x%02X (bus %d) nonce=0x%08X ***\n",
                            slaves[i].address, slaves[i].bus, nonce);
                    } else {
                        Serial.printf("hit by 0x%02X nonce=0x%08X (display-diff, not submitted)\n",
                            slaves[i].address, nonce);
                    }
...
void setup() {
    ...
    difficultyToTarget(displayDifficulty, shareTarget);  // easy target -> slaves mine this
    difficultyToTarget(poolDifficulty, poolTargetBE);     // real target -> submission gate
